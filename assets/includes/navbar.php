...
if (!defined('BASE_URL')) {
    $_baseUrl = getenv('BASE_URL');
    if (!$_baseUrl || !is_string($_baseUrl)) $_baseUrl = '/';
    define('BASE_URL', $_baseUrl);
}
?>
<!-- Modern Navbar -->
<?php
// Role checking helper for navigation
$userRole = $_SESSION['role'] ?? 'GUEST';
$canManageCertificates = in_array($userRole, ['SUPERADMIN', 'ADMIN', 'COORDINATOR']);
$canUploadData = in_array($userRole, ['SUPERADMIN', 'ADMIN']);
?>
<nav class="navbar navbar-expand-lg navbar-modern fixed-top">
    <div class="container-fluid">
        <button class="btn btn-sidebar-toggle d-lg-none me-2" id="sidebarToggle" aria-label="Toggle sidebar">
        ...
        <div class="collapse navbar-collapse" id="navMenu">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle <?= strpos($_SERVER['REQUEST_URI'], 'upload') !== false || strpos($_SERVER['REQUEST_URI'], 'setup') !== false ? 'active' : '' ?>" href="#" id="adminMenu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="userMenu" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="fas fa-user-circle me-1"></i><?php
                        echo $_SESSION['name'] ??
                            (is_array($_SESSION['user'] ?? null) ? ($_SESSION['user']['first_name'] ?? 'User') : 'User');
                        ?>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userMenu">
                        ...
                        <?php if (!empty($_SESSION['sso_login'])): ?>
                            <!-- Back to SSO link for SSO users -->
                            <li>
